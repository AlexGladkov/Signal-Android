 ...
 */
public class DetailStaggeredAdapter extends RecyclerView.Adapter<DetailStaggeredAdapter.ViewHolder> {
    private final String TAG = DetailStaggeredAdapter.class.getSimpleName();
    ...
    public DetailStaggeredAdapter(ArrayList<ImageModel> models, AppCompatActivity mActivity,
    ...
    @Override
    public DetailStaggeredAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        CellImageStaggeredBinding binding = CellImageStaggeredBinding.inflate(inflater, parent, false);
        return new DetailStaggeredAdapter.ViewHolder(binding.getRoot());
    }

    @Override
    public void onBindViewHolder(final DetailStaggeredAdapter.ViewHolder holder, int position) {
        final ImageModel model = models.get(position);
        int containerWidth = Utilities.getScreenWidth(mActivity);
        int containerHeight = (int) (containerWidth * 0.617);
