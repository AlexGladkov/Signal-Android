...
//            indicatorColor = typedArray.getColor(R.styleable.SimpleTabBar_indicatorColor, defaultIndicatorColor);
            ...
            imageTint = typedArray.getColor(R.styleable.SimpleTabBar_imageTint, 0);
...
//        parent.setBackgroundColor(getResources().getColor(R.color.blackPrimary));
...
//        view.setMinimumHeight(tabHeight);
    ...
    private View getTabViewWithIcon(SimpleTabBarAdapter adapter, int position) {
        LinearLayout tabView = (LinearLayout) LayoutInflater.from(context).inflate(R.layout.tab_item, null);

        TextView tv = (TextView) tabView.findViewById(R.id.tab_txt);
        ImageView iv = (ImageView) tabView.findViewById(R.id.tab_iv);
        ...
        if (imageTint != 0) iv.setColorFilter(getResources().getColor(imageTint));
        if (imagePosition == 1) {
